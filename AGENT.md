...
- **Model tiering:** the `flash` tier by default; the `pro` tier only for heavy reasoning (script reads, final synthesis, recut). Each agent's entry in Section 4 names its tier as "Flash" or "Pro" — a tier, not a provider.
- **One provider chain.** Any of Cerebras, Groq, Gemini or a local Ollama serves a call, tried in the order `LUMEN_LLM_PROVIDERS` gives; a provider with no credentials is skipped, so the same setting works on a laptop and on the deploy. Each provider names its own pair of models, and one that offers nothing heavier serves `pro` from its flash model. All of it is `backend/services/llm.py`, the only place a model is called.
- **Structured output:** every agent returns validated JSON, asked for as JSON by every provider. Never parse prose. A provider can also be asked to *enforce* a shape (`llm.generate_json_traced(schema=...)`, a schema derived from the agent's own sample output by `llm.schema_from_example`), and no agent does yet: whether it earns the coupling is what `backend/eval/structured` measures. Schema dialects differ — Gemini's `response_schema` refuses the `additionalProperties` that OpenAI-style strict mode requires — so `llm._schema_for` translates per provider.
- **Repairs are counted.** A live reply is coerced field by field through `core/llm_output.py` before an agent uses it, and `llm_output.watching()` counts those repairs for a block of work the way `llm.recording()` counts calls. An offline run repairs nothing, by construction, so a non-zero count means a live reply arrived in the wrong shape.
- **Cost:** keep model inputs small (clipped script reads, screening packets), batch where possible. Everything runs on free plans: plain JSON requests only, with no code for the paid-only features (Google Search grounding, image generation) or Vertex AI. A rate-limited call waits as long as the provider asks (up to 20 s), then moves to the next candidate — the same provider's next model, or the next provider; a timed-out call (504) moves on at once.
- **Live, or sample output.** Every call's trace says which provider and model answered and whether a model answered at all. Ask that with `llm.is_live(trace)` and never by comparing a provider name, or the day a second provider arrives its answers get reported as Lumen's sample output.
